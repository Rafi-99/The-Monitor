                ...
                if (Objects.requireNonNull(c.getEvent().getMember().getVoiceState()).getChannel() != null && Objects.requireNonNull(c.getEvent().getMember().getVoiceState()).getChannel() == Constants.getAudioManager(c).getConnectedChannel() && Constants.getAudioManager(c).isConnected()) {
                    try {
                        ...
                        if (playerVolume >= 0 && playerVolume <= 200) {
                            ...
                            c.getEvent().getChannel().sendMessage("The player volume has been set to "+ Constants.getMusicManager(c).player.getVolume() + "%").setMessageReference(c.getEvent().getMessage()).mentionRepliedUser(false).queue();
                            return;
                        }
                        c.getEvent().getChannel().sendTyping().queue();
                        c.getEvent().getChannel().sendMessage("Please enter a number between 0 and 200 inclusive.").setMessageReference(c.getEvent().getMessage()).mentionRepliedUser(false).queue();
                    }
                    catch (Exception e) {
                        c.getEvent().getChannel().sendTyping().queue();
                        c.getEvent().getChannel().sendMessage("Please enter a number between 0 and 200 inclusive.").setMessageReference(c.getEvent().getMessage()).mentionRepliedUser(false).queue();
                    ...
                    c.getEvent().getChannel().sendTyping().queue();
                    c.getEvent().getChannel().sendMessage("You have to be in the same voice channel as me to change the player volume.").setMessageReference(c.getEvent().getMessage()).mentionRepliedUser(false).queue();
                }
            }
        }
    }
