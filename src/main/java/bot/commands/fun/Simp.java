...
public class Simp implements CommandInterface {
    ...
    public void handle(CommandContext c) {
        if (c.getCommandParameters().size() == 1) {
            ...
            c.getEvent().getChannel().sendMessage("Type in " + Constants.getCurrentPrefix(c) + "simp and mention someone to use this command!").setMessageReference(c.getEvent().getMessage()).mentionRepliedUser(false).queue();
        }
    }
